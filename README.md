...
| Table | Columns |
|---|---|
| `published_posts` | post_id (PK), draft_id (UNIQUE), title, author_name, tags_summary, published_at |
...
### 3.6 - Create necessary variables for post creation

Add a `Declare Variable` statement node to create the following variables needed for post creation:
- **Name:** `postId`
- **Type:** `string`
- **Expression:** `uuid:createType4AsString()`

- **Name:** `tagsSummary`
- **Type:** `string`
- **Expression:** `string:'join(",", ...draft.tags.map(tag => tag.label))`

- **Name:** `publishedAt`
- **Type:** `time:Utc`
- **Expression:** `time:utcNow()`

### 3.7 — Create the published post in Publishing DB

Add an `Insert row into published_posts` action node from the `publishingDB` connection. Toggle the **Array** option and click `+ Add New Item`, then set the row value:
...
### 3.8 — Update the draft status in Content DB
